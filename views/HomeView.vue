<!-- Vista congelada temporal
Causa poco avance en depuracion, podria clausurarse es un plus para el usuario
detalles: El compilador muestras pistas con poco valor real -->
<template>
		...
		<section class="hero-section max-w-7xl mx-auto mb-12"> 
			<div class="text-center mb-8">
				 ...
				 <div class="flex items-center justify-center gap-4 mb-4">
					 <div class="w-16 h-16 bg-gradient-to-br from-green-600 to-green-800 rounded-xl flex items-center justify-center shadow-lg">
				...
				<div class="max-w-2xl mx-auto mt-8">
					<div class="relative">
						<input type="text"   placeholder=" Buscar: materiales, autores, categorías."
						 ...
						 <button class="absolute right-2 top-1/2-translate-y-1/2 px-6 py-2 bg-blue-500 text-white roundend-xl hover:bg-blue-600 transition-colors font-medium">
								 Buscar
						 </button>
					</div>
		...
		<section class="max-w-7xl mx-auto mb-12">
			<div class="flex items-center justify-between mb-6">
				...
				<div class="flex gap-2">
					...
					:class="featuredTab === 'recent'
						? 'bg-blue-500 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
					">Recientes
					</button>
					<button class="px-4 py-2 rounded-lg transition-colors"
						 :class="featuredTab === 'popular'
						 ? 'bg-blue-500 text-white'
						 : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'"
						>Populares
					</button>
			...
			<div v-if="featuredMaterials.length === 0" class="text-center py-12">
				<svg class="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stoke="currentColor" viewBox="0 0 24 24">
					 <path  stroke-linecap="round" stroke-linejoin="round" stroke-width="2"  d="M9 12h6m-6 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0
					01.293.707V19a2 2 0 01-2 2z"/> <!--autoclausura/cierre> -->
				</svg>
		...
		<section class="max-w-7xl max-auto">
				<div class="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg border border-gray-200 dark:border-gray-700">
					<div>
						...
						<p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Almacenamiento</p>
					</div>
				</div>
			</section>
...
<script setup lang="ts">
 ...
 import {MaterialBseService} from '@/services/materials/MaterialBaseService';  // adaptar a herencia de materials
 import { ToastServiceClass } from '@/services/ToastService';  // adecuarlo con src_nuevo  (switch controlado)
	// Componentes que aún no han sido creados
 import StatsCard from '@/components/port_widgets/StatsCard.vue';
 import QuickActionCard from '@/components/port_widgets/QuickActionCard.vue';
 ...
 import ReportSummaryModal from '@/components/ReporteSummaryModal.vue';
	import { Material } from '@/interfaces/interfacesToast.ts';  //No ha sido declarado/
	...
	// Exp. visual comoda p/usuario final los materiales estan alcance (segun su uso->favoritos)
	const quickActions = computed(()=> {
		const baseActions = [
			...
			// ln 267.Spread la inicializacion de los valores esta siendo expandida
		if(authStore.role === 'student'){
			...
			// ln 275.Spread la inicializacion de los valores esta siendo expandida
		...
		try{
			...
			const totalsizeMB = materials.reduce((acc,m)=> acc + (m.tamanioMB || 0), 0);
			 stats.value.storageUsedGB = totalsizeMB /1024;
				...
				/*Agregar datos reales, segun los usuarios activos [dts reales cuando el sist este en prod] */
	...
	function handleQuickSearch(){
		if(quickSearchQuery.value.trim()){
			...
			console.log('Pensando el logica final..');
	...
	function viewSavedSearches() {
		console.log('invocando propiedades necesarias para guardar valores en ellas');

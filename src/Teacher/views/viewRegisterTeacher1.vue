  	...
  	// Observa authStore.user y store_profile.socialUser, cuando user.uid intenta guardar el perfil
  	 ...
  	 async function onSumbitTraditional() {
  	 	...
  	 	try{
  	 		...
  	 		const     uid = created?.uuid;
  	 		  if (!uid) throw naw Error('Lo sentimos no se creo el usuario (uid):',uid, 'corresp al E-mail:',tradicional.email);
  	 ...
  	 async function onSubmitProfile() {
  	 	...
  	 	try{
  	 		...
  	 		store_profile.error = err?.message ?? 'Error al cargar el perfil';
  	 	}
  	}

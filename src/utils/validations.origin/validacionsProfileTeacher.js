export const validacionesPerfilProfessor = {
 		...
 		try{
 			...
 			const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
 			 ...
 			 const valido = regex.test(email.trim());
 			  if (!valido) throw new Error('El e-mail no es valido');
 	...
 	sugeryforPasswordChange(fechaUltimoCambio){
 		try{
 			if (!(fechaUltimoCambio intanceof Date) ){
 				throw new Error("La Fecha no válida"); //si existe ecepsion son nec. llaves
 			}

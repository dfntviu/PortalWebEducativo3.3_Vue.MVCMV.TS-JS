    const materialesValidations = {
      ...
      existsInCollection: async function(id, collection, service) {
        try {
      ...
      sanitizeInput: function(str) {
        ...
        if (!str) return "";
        return str.replace(/<[^>]*?>/gm, "").replace(/<\/?[^>]*>/g, ""); // Elimina HTML
      },

      // --- Otras Validaciones de MATERIALES & Avanzados ---
      validateForm: function(str, minLength = 3) {
   ...
   /** *@type{ { Habilitar unicamente con TS semipuro

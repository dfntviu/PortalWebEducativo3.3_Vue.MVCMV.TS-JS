<template>
  <div class="student-materials-container">
    ...
    <div class="card-ext-student main-card">
      ...
      <div v-if="authStore.user?.email" class="upload-section">
...
<script setup lang="ts">
   ...
   // =================================
	 ...
	 const tagsInpus = ref(''); //temporal para tags
	 ...
	 */
	  ...
	  // =====================
		function processTags(): void{
		 	if (tagsInpus.value.trim) {
		 		form.value.tags = tagsInpus.value.split(',')
		 						  .map(tag => tag.trim()
	...
	async function handleEdit(material: Material): void {
		...
		try{
           ...
           return;
			 ...
			 tagsInpus.value = material.tags?.join(', ') || '';
			 pdfBase64.value = null;
			 file.value = null
  ...
  function updateMaterial(): Promise<void> {
    try{
  ...
  async function createMaterial(): Promise<void> {
	...
	function cancelEdit():void{
	  ...
	  function handleDelete(materialId: Material){
		...
		function scrollFrom():void {
			...
			if (formElement) {
				formElement.scrollIntoView({be	vior: 'smoth', block: 'start'}):
			}
		}

    function formDate(date: any): string{
